...
class ChatSerializer(serializers.ModelSerializer):
    ...
    admins = serializers.SerializerMethodField(source='get_admins')  # read-only field по дефолту

    class Meta:
        model = Chat
        fields = ('title', 'description', 'creator','category', 'users', 'admins')
    
    def get_users(self, instance):    # мб докинуть какие-то проверки чтоб если данных не было таких - не выбрасывало ошибку
        members = ChatMember.objects.filter(chat_id=instance.pk)
        users = [User.objects.get(id=member.user_id) for member in members]
        return ChatUserSerializer(instance=users, context=self.context, many=True).data

    def get_admins(self, instance):
        members = ChatMember.objects.filter(chat_id=instance.pk, is_admin=True)
        admins = [User.objects.get(id=member.user_id) for member in members]
